...
class PHPAnalyzer:
    ...
    def analyze(self, filepath: str, content: str, compiled_rules: List) -> List[Finding]:
        findings = []
        clean = strip_comments_and_strings(content, '.php')
        findings.extend(self.lexer_analyzer.analyze(filepath, content, '.php'))
        if compiled_rules:
            findings.extend(scan_content_for_rules(
                filepath, compiled_rules, ContentContext(clean)))
        findings.extend(self.taint_tracker.analyze(filepath, content, '.php'))
        return findings